...
/*if(isset($_GET['crit_name']) && !empty($_GET['crit_name'])) {*/
if(isset($_GET['crit_name']) && !empty($_GET['crit_name'])) {
    $crit_search = $_GET['crit_name'];
    // $crit_search = $_GET['crit_name'];
    ...
    try
    {
        ...
        switch ($crit_search) {
            case "n_arch":
                if(isset($_GET['nom_arch']) && !empty($_GET['nom_arch'])) {
                    // if(isset($_GET['nom_arch']) && !empty($_GET['nom_arch'])) { // si le nom de l'archive existe
                    // echo "nom_arch= ".$_GET['nom_arch']."<br/>";
                    $tmp_n_arch = check_archive_exist_by_nom_archive($connexion, $_GET['nom_arch']);
                    // $tmp_n_arch = check_archive_exist_by_nom_archive($connexion, $_GET['nom_arch']);
                    if($tmp_n_arch["result"]) {
                        ...
                        $full_data_archive["message"] = "erreur de suppression de l'archive par nom d'archive ou ce dernier n'exite pas";
                    }
                }
                else {
                ...
                break;
            case "n_edi":
                if(isset($_GET['nom_edi']) && !empty($_GET['nom_edi'])) {
                    $tmp_n_edi = check_archive_exist_by_nom_edifice($connexion, $_GET['nom_edi']);
                    if($tmp_n_edi["result"]) {
                        ...
                        $full_data_archive["message"] = "erreur de suppression de l'archive par nom edifice ou ce dernier n'exite pas";
                    }
                }
                else {
                    $full_data_archive["message"] = "le paramètre nom_edificei n'est pas correct";
                }
                break;
            case "p_arch":
                if(isset($_GET['prop_arch']) && !empty($_GET['prop_arch'])) {

                    $del = true;
                    $tmp_p_arch = get_all_id_archive_of_id_prop_arch($connexion, $_GET['prop_arch']);
                    if($tmp_p_arch["result"]) {
                        ...
                        $full_data_archive["message"] = "erreur de suppression de l'archive par nom proprietaire ou ce dernier n'exite pas";
                    }
                }
                else {
                ...
                break;
            case "t1_arch":
                if(isset($_GET['t1_arch']) && !empty($_GET['t1_arch'])) {

                    $del = true;
                    $tmp_t1_arch = get_all_id_archive_of_type_primaire($connexion, $_GET['t1_arch']);
                    if($tmp_t1_arch["result"]) {
                        ...
                        $full_data_archive["message"] = "erreur de suppression de l'archive par type primaire ou ce dernier n'exite pas";
                    }
                }
                else {
                ...
                break;
            case "t2_arch":
                if(isset($_GET['t2_arch']) && !empty($_GET['t2_arch'])) {
                    $del = true;
                    $tmp_t2_arch = get_all_id_archive_of_type_secondaire($connexion, $_GET['t2_arch']);
                    if($tmp_t2_arch["result"]) {
                        ...
                        $full_data_archive["message"] = "erreur de suppression de l'archive par type secondaire ou ce dernier n'exite pas";
                    }
                }
                else {
                ...
                break;
            case "an":
                if(isset($_GET['an_arch']) && !empty($_GET['an_arch'])) {
                    $del = true;
                    $tmp_an_arch = get_all_id_archive_of_annee($connexion, $_GET['an_arch']);
                    if($tmp_an_arch["result"]) {
                        ...
                        $full_data_archive["message"] = "erreur de suppression de l'archive par annee ou cette derniere n'exite pas";
                    }
                }
                else {
